...
trait Elements
{
    ...
    protected function readInheritedProps()
    {
        ...
        if ($this->arParams['ELEMENT_ID'])
        {
            ...
            if (!$this->arResult['SEO_TAGS']['PAGE_TITLE'])
            {
                $this->arResult['SEO_TAGS']['PAGE_TITLE'] = $seoValues['ELEMENT_PAGE_TITLE'];
            }
        ...
        elseif ($this->arParams['SECTION_ID'])
        {
            ...
            if (!$this->arResult['SEO_TAGS']['PAGE_TITLE'])
            {
                $this->arResult['SEO_TAGS']['PAGE_TITLE'] = $seoValues['SECTION_PAGE_TITLE'];
            }
    ...
    protected function setSeoTags()
    {
        ...
        if ($this->arResult['SEO_TAGS']['PAGE_TITLE'])
        {
            ...
            $APPLICATION->SetTitle($this->arResult['SEO_TAGS']['PAGE_TITLE']);
        }
    ...
    protected function processingElementsResult($element)
    {
        ...
        if ($this->arParams['RESULT_PROCESSING_MODE'] === 'EXTENDED')
        {
            ...
            $this->arParams['SELECT_PROPS'];
            $arElement['PROPS'] = $element->GetProperties();
            foreach ($arElement['PROPS'] as $code => $arProp) {
                if (!in_array($code, $this->arParams['SELECT_PROPS'])) {
                    unset($arElement['PROPS'][$code]);
                }
            }
        }
